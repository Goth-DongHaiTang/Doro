...
async function sendMessage() {
  if (!command.value || !apiKey.value) return;
  // 添加用户消息
  messages.value.push({ role: 'user', content: command.value });
  const userPrompt = command.value;
  command.value = '';
  ...
  try {
    ...
    await scrollToBottom();
    const reply = await sendToDeepseek(userPrompt, apiKey.value);
    // 替换最后的 loading 文本
    messages.value[messages.value.length - 1].content = reply;
  } catch (e) {
    messages.value[messages.value.length - 1].content = '请求失败，请检查 API Key 或网络';
  }
  await scrollToBottom();
}
  ...
  <main class="relative flex flex-col items-center justify-start min-h-screen p-8"
    ...
    <div :class="[
      ...
      <div class="p-4 flex flex-col">
        ...
        <button class="mt-4 px-4 py-2 bg-gray-300 rounded hover:bg-gray-400" @click="saveApiKey">保存</button>
        <button class="mt-4 px-4 py-2 bg-red-300 rounded hover:bg-red-400" @click="toggleSidebar">关闭侧边栏</button>
      </div>
    </div>

    <!-- 聊天区 -->
    <div class="w-full max-w-md flex-1 overflow-y-auto p-2 border border-gray-300 rounded mb-4"
    ...
    <div class="flex w-full max-w-md">
      ...
      <button @click="sendMessage"
        class="px-4 bg-blue-500 text-white rounded-r hover:bg-blue-600 transition-colors">发送</button>
    </div>
